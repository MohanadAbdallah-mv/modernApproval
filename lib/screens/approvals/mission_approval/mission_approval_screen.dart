...
import 'package:modernapproval/widgets/mission/mission_details_bottom_sheet.dart';
...
class _MissionApprovalScreenState extends State<MissionApprovalScreen> {
  ...
  late Future<List<MissionApprovalItem>> _requestsFuture;

  String _reasonFilter = '';
  DateTime? _selectedDate;
  List<MissionApprovalItem> _filteredRequests = [];
  ...
  void _fetchData() {
    setState(() {
      _requestsFuture = _apiService.getMission(
  ...
  void _extractVacationTypes(List<MissionApprovalItem> requests) {
    final vcTypesNames =
        requests
    ...
    _availableReasons = [''] + vcTypesNames;
  }

  void _applyFilters(List<MissionApprovalItem> allRequests) {
    setState(() {
      _filteredRequests =
          allRequests.where((request) {
            // If both filters are empty, show all
            if (_reasonFilter.isEmpty && _selectedDate == null) {
              return true;
            }

            bool matchesReason = true;
            bool matchesDate = true;

            // Apply reason filter only if it's not empty
            if (_reasonFilter.isNotEmpty) {
              matchesReason = (request.reasonAr ?? '') == _reasonFilter;
            }
            ...
            if (_selectedDate != null) {
              matchesDate =
                  ...
                  _isSameDate(request.enterDate!, _selectedDate!);
            }
  ...
  Widget _buildFilterWidget() {
    return Row(
      ...
      children: [
        ...
        if (_reasonFilter.isNotEmpty || _selectedDate != null)
          IconButton(
            ...
            child: InputDecorator(
              ...
              child: DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  value: _reasonFilter.isEmpty ? null : _reasonFilter,
                  isExpanded: true,
                  icon: Icon(
                  ...
                  items:
                      _availableReasons.map((String reason) {
                        return DropdownMenuItem<String>(
                          value: reason.isEmpty ? null : reason,
                          child: Text(
                            reason.isEmpty ? l.translate('all_types') : reason,
                            style: TextStyle(
                              color:
                                  reason.isEmpty
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar(
        title: l.translate('missionApproval'),
      ...
      body: Column(
        children: [
          ...
          // List content
          Expanded(
            child: FutureBuilder<List<MissionApprovalItem>>(
              ...
              builder: (context, snapshot) {
                ...
                final displayRequests =
                    _showFilters &&
                            (_reasonFilter.isNotEmpty || _selectedDate != null)
                        ? _filteredRequests
                        : allRequests;
