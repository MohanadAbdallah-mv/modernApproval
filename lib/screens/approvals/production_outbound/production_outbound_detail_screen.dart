...
class ProductionOutboundDetailScreen extends StatefulWidget {
  final UserModel user;
  final ProductionOutbound request;

  const ProductionOutboundDetailScreen({
    super.key,
    required this.user,
  ...
  @override
  State<ProductionOutboundDetailScreen> createState() =>
      _ProductionOutboundDetailScreenState();
}

class _ProductionOutboundDetailScreenState
    extends State<ProductionOutboundDetailScreen> {
  final ApiService _apiService = ApiService();
  late Future<Map<String, dynamic>> _detailsFuture;

  ProductionOutboundMaster? _masterData;
  List<ProductionOutboundDetail>? _detailData;
  ...
  Future<Map<String, dynamic>> _loadAllDetails() async {
    try {
      final results = await Future.wait([
        _apiService.getProductionOutboundMaster(
          trnsTypeCode: widget.request.trnsTypeCode,
          trnsSerial: widget.request.trnsSerial,
        ),
        _apiService.getProductionOutboundDetail(
          trnsTypeCode: widget.request.trnsTypeCode,
          trnsSerial: widget.request.trnsSerial,
        ),
      ]);
      ...
      setState(() {
        _masterData = results[0] as ProductionOutboundMaster;
        _detailData = results[1] as List<ProductionOutboundDetail>;
      });
  ...
  pw.Widget _buildFixedPdfFooter(
    pw.Font ttf,
    List<ProductionOutboundDetail> productionOutboundList,
  ) {
